...
class DownloadWorker(Worker):
    ...
    def do_task(self, filepath, url, directory, retry, timeout):
        with closing(
            self.session.get("%s/%s" % (url, filepath), allow_redirects=False, stream=True, timeout=timeout)
        ) as response:
...
def fetch_git(url, directory, jobs, retry, timeout):
    ...
    if response.status_code == 200 and is_html(response) and "HEAD" in get_indexed_files(response):
        ...
        process_tasks([".git/", ".gitignore"], RecursiveDownloadWorker, jobs, args=(url, directory, retry, timeout))
    ...
    process_tasks(tasks, FindRefsWorker, jobs, args=(url, directory, retry, timeout))
    ...
    process_tasks(objs, FindObjectsWorker, jobs, args=(url, directory, retry, timeout), tasks_done=packed_objs)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        usage="%(prog)s [options] URL DIR", description="Dump a git repository from a website."
    )
